...
public:
                         ...
                         double max_search_distance, double radius, double step_size)
  {
    ...
    if (min_distance_active && max_distance_active)
    {
      distance_gain = std::min(1 * (closest_distance.first - min_distance), max_distance - closest_distance.second);
    }
    else if (min_distance_active)
    {
      distance_gain = 1 * (closest_distance.first - min_distance);
    }
    else
    {
              ...
              double step_size, std::map<int, std::pair<geometry_msgs::Pose, double>> routers, bool routers_active)
  {
    ...
    cost_ = (distance4d(parent_) *
             std::fmax(std::exp(-ltl_lambda * std::min(distance_gain, max_router_difference)), 1)) +
            parent_->cost(rtree, ltl_lambda, min_distance, max_distance, min_distance_active, max_distance_active,
...
#pragma omp parallel for
  ...
  double distance4d(std::shared_ptr<RRTNode> other)
  {
    double yaw_diff_scaler = 4.0;

    Eigen::Vector4d p4(state_[0], state_[1], state_[2], state_[3]);
    Eigen::Vector4d q4(other->state_[0], other->state_[1], other->state_[2], other->state_[3]);
    Eigen::Vector4d diff = p4 - q4;
    // diff[3] *= yaw_diff_scaler;
    diff[3] = std::pow(diff[3], yaw_diff_scaler);
    return (diff).norm();
  }
};
}  // namespace stl_aeplanner
